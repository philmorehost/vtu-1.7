...
class ModularApiGateway {
    ...
    /**
     * Get a list of available exam cards
     */
    public function getAvailableExamCards() {
        try {
            $stmt = $this->pdo->prepare("
                SELECT id as card_type_id, name as card_name, selling_price as unit_amount
                FROM service_products
                WHERE service_type = 'exam' AND status = 'active'
            ");
            $stmt->execute();
            ...
            error_log("Exam Card Fetch Error: " . $e->getMessage());
            return ['success' => false, 'message' => 'Could not retrieve exam card list.'];
        }
    }
     ...
     */
    public function purchaseExamCard($cardTypeId, $quantity) {
        $providerConfig = $this->getProvider('exam', null, $cardTypeId);

        if (!$providerConfig) {
            error_log("Exam Card Provider Error: No provider found for card_type_id: " . $cardTypeId);
            return [
                'success' => false,
                'message' => 'No API provider available for this exam card'
            ];
        }
