<?php
/**
 * Modular Gift Card API Endpoint
 * Handles gift card purchases through modular providers
 */
...
try {
    ...
    if (!$cardType || !$amount || !is_numeric($amount) || $amount <= 0 || $quantity <= 0) {
    ...
    if (!$providerConfig) {
        throw new Exception('No active gift card provider available');
    }
    ...
    // Check if provider supports gift cards
    if (!in_array('gift_card', $provider->getSupportedServices())) {
        throw new Exception('Provider does not support gift card service');
    }
    ...
    try {
        // Process gift card purchase through provider
        $result = $provider->purchaseGiftCard($cardType, $amount, $quantity);
        ...
        // Record transaction
        $description = "Purchase {$quantity}x {$cardType} gift card(s) of {$amount} each via {$providerConfig['display_name']}";
        ...
        $stmt = $pdo->prepare("
            ...
            VALUES (?, ?, ?, ?, 'Completed', ?, ?, ?, ?)
        ");
        $stmt->execute([$userId, $cardType . ' Gift Card', $description, -$totalAmount, $serviceDetails, $source, $balance_before, $balance_after]);
        ...
        $logStmt->execute([
            ...
            $result['success'],
            $result['message'],
            json_encode(['card_type' => $cardType, 'amount' => $amount, 'quantity' => $quantity]),
            json_encode($result)
        ]);
        ...
        echo json_encode([
            'success' => true,
            'message' => "Gift card(s) purchased successfully via {$providerConfig['display_name']}",
            'data' => $result['data'],
            'transaction_id' => $result['transaction_id'],
            'provider' => $providerConfig['display_name'],
